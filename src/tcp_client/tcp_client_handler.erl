...
%% 发数据包
handle_cast({send_socket_msg, Cmd, InfoBin}, #socket_info_record{config_behavior = ConfigBehavior, socket = Socket} = State) ->
  tcp_client_send:send_data_by_socket(Socket, Cmd, InfoBin, ConfigBehavior),
  {noreply, State};
