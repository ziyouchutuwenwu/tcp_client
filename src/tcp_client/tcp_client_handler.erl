...
-export([start_link/2, send_socket_msg/3]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2, code_change/3]).

-record(socket_info_record, {config_behavior, socket, ip, port, recv_timer_ref, recv_timeout_count}).

start_link({Ip, Port}, ConfigBehavior) ->
	gen_server:start_link(?MODULE, [{Ip, Port}, ConfigBehavior], []).

init([{Ip, Port}, ConfigBehavior]) ->
	{ok, #socket_info_record{config_behavior = ConfigBehavior, ip = Ip, port = Port}, 0}.
...
handle_info({tcp, _Socket, Data}, #socket_info_record{config_behavior = ConfigBehavior, recv_timer_ref = OldRecvTimerRef} = State) ->
	...
	SocketHandlerModule:on_server_data(Cmd, InfoBin),
...
handle_info({tcp_closed, _Socket}, #socket_info_record{config_behavior = ConfigBehavior, ip = Ip} = State) ->
	%%服务器断开连接
	SocketHandlerMod = ConfigBehavior:get_socket_handler_module(),
	SocketHandlerMod:on_disconnected(Ip),
...
handle_info(timeout, #socket_info_record{config_behavior = ConfigBehavior, ip = Ip, port = Port} = State) ->
	OptionsModule = ConfigBehavior:get_socket_options_module(),
	...
	case gen_tcp:connect(Ip, Port, Options, OptionsModule:get_tcp_conn_timeout()) of
		{ok, Socket} ->
			...
			SocketHandlerModule = ConfigBehavior:get_socket_handler_module(),
			SocketHandlerModule:on_server_connected(IP),
...
send_socket_msg(Cmd, InfoBin, #socket_info_record{config_behavior = ConfigBehavior, socket = Socket} = State) ->
